...
const Links = () => {
  ...
  return (
    <div ref={container} className="flex relative w-full h-fit justify-between">
      <div ref={selector} className="absolute pointer-events-none">
        <Image
          src="/assets/icons/border.svg"
          alt="selector"
          className="mt-[0.5vh] ml-[5vw] scale-150"
          width={222}
          height={352}
        />
      </div>
      <div className="text-[1.5vw] font-bold uppercase flex flex-col items-start">
        {links.map((link, index) => (
          <div key={index} ref={link.ref} className="h-[5vh] ml-[3vw] flex items-center justify-center cursor-pointer">
            {link.text}
          </div>
        ))}
      </div>
